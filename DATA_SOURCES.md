...
| **Expansion pull** | **2016-17 → 2019-20 regular seasons + 2016 … 2023 playoffs** | Acquired 2026-09-01 from SRC-SHUFINSKIY (`scripts/fetch_shufinskiy.py --plan cycle1`, same pinned commit). Earlier RS extends the ladder toward the master-plan 8–10 season target; **promotion to binding coverage is gated on the combined re-ingest's data-quality checks**. Playoffs 2016–2023 are phase-labelled and **not** pooled into RS training. The **2024-25 playoffs stay held out** for the transport test. Also pulled: `cdnnba` / `nbastatsv3` / `matchups` surfaces for 2020-24 + 2025 (surface diversification; `matchups` → defensive-side follow-up). |
| **Playoffs** | Ingest and **label** for all seasons; **exclude from cycle-1 training by default**, reserving them for transport / robustness evaluation (contract §29 Q3). | Keeps regular season and playoffs from being silently pooled (master plan §6.3); leaves the playoff-transport test clean. |
